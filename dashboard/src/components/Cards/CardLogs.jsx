    ...
    try {
      ...
      const [resLogs, resCount, resActions] = await Promise.all([
       fetch(`${API_URL}/`, { headers }),
        ...
        fetch(`${API_URL}/count/acao`, { headers }),
      ]);

      if (resLogs.status === 401 || resCount.status === 401 || resActions.status === 401) {
        toast.error("Não autorizado — faça login como administrador");
        setLogs([]);
        setStats({ total: 0, criados: 0, atualizados: 0, deletados: 0 });
        return;
      }
    ...
    <div>
      ...
      <AnimationWrapper delay={0.3}>
        <Card className="mt-10 hover:border-sky-400 dark:hover:border-sky-950">
          <CardHeader>
            ...
            <div className="flex items-center gap-3 mb-8">
              <div className="relative w-full">
                ...
                <input
                  className="w-full pl-11 pr-3 py-2 rounded-lg border bg-background text-foreground focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all shadow-sm"
                ...
                <tbody className="divide-y divide-border">
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-muted/10 text-foreground">
                      ...
                      <td className="px-4 py-2 text-sm">{new Date(log.data).toLocaleString("pt-BR")}</td>
          ...
          <Separator />
          <PaginationDemo currentPage={page} totalPages={totalPages} onChangePage={(p) => setPage(p)} />
        </Card>
      </AnimationWrapper>
    </div>
  );
