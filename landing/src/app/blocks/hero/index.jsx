"use client";
import Image from "next/image";
import Link from "next/link";
import { ArrowUpRight, ArrowDown } from "lucide-react";
import { motion } from "framer-motion";
import { Button } from "@/components/ui/button";

const Main = ({
  heading = "Monitoramento de água",
  subheading = " em tempo real",
  description = "Sistema inteligente que detecta vazamentos, reduz desperdícios e ajuda você a economizar até 40% na conta de água",
  buttons = {
    primary: {
      text: "Começar Agora",
      url: "#contato",
    },
    secondary: {
      text: "Veja os planos disponíveis",
      url: "#planos",
    },
  },
  image = {
    src: "/interface-app.png", 
    alt: "Interface do aplicativo de monitoramento de água",
  },
}) => {
  return (
    <>
      <motion.section
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, delay: 0.2 }}
        className="py-20 lg:py-12"
        aria-labelledby="section-heading"
      >
        <div className="container flex flex-col items-center gap-10 lg:my-0 lg:flex-row">
          <div className="flex flex-col gap-7 lg:w-2/3">
            <h1
              id="section-heading"
              className="text-5xl font-semibold text-foreground md:text-5xl lg:text-8xl"
            >
              <span className="mx-auto container">{heading}</span>
              <span className="text-[#0084d1]">{subheading}</span>
            </h1>

            <p className="text-base text-muted-foreground md:text-lg lg:text-xl">
              {description}
            </p>

            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: 0.2 }}
              className="flex flex-wrap gap-6 text-sm"
              aria-label="Recursos principais do sistema"
            >
              <div className="flex items-center gap-2 text-slate-700 dark:text-white">
                <span className="w-2 h-2 bg-emerald-500 rounded-full" />
                <span>Detecção automática de vazamentos</span>
              </div>
              <div className="flex items-center gap-2 text-slate-700 dark:text-white">
                <span className="w-2 h-2 bg-blue-500 rounded-full" />
                <span>Alertas em tempo real</span>
              </div>
              <div className="flex items-center gap-2 text-slate-700 dark:text-white">
                <span className="w-2 h-2 bg-purple-500 rounded-full" />
                <span>Economia comprovada</span>
              </div>
            </motion.div>

            <div className="flex items-start gap-5 lg:gap-7">
              <Button asChild aria-label="Ir para contato">
                <Link href={buttons.primary?.url}>
                  <div className="flex items-center gap-2">
                    <ArrowUpRight className="size-4" aria-hidden="true" />
                  </div>
                  <span className="pr-6 pl-4 text-sm whitespace-nowrap lg:pr-8 lg:pl-6 lg:text-base">
                    {buttons.primary?.text}
                  </span>
                </Link>
              </Button>

              <Button asChild variant="link" className="underline" aria-label="Ver planos disponíveis">
                <Link href={buttons.secondary?.url}>{buttons.secondary?.text}</Link>
              </Button>
            </div>
          </div>

          <div className="relative z-10">
            <div className="absolute top-2.5 left-1/2 h-[92%] w-[69%] -translate-x-[52%] overflow-hidden rounded-[35px]">
              <Image
                src={image.src}
                alt={image.alt}
                fill
                priority
                sizes="(max-width: 768px) 100vw, 50vw"
                className="object-cover object-[50%_0%]"
              />
            </div>
            <Image
              className="relative z-10"
              src="https://deifkwefumgah.cloudfront.net/shadcnblocks/block/mockups/phone-2.png"
              width={450}
              height={889}
              alt="Mockup de smartphone exibindo o aplicativo de monitoramento de água"
              priority
              decoding="async"
            />
          </div>
        </div>
        <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 1 }}
        className=" flex flex-col items-center gap-2"
        aria-label="Indicador para rolar a página"
      >
        <motion.div
          animate={{ y: [0, 10, 0] }}
          transition={{ duration: 1.5, repeat: Infinity }}
          className="mt-2 md:mt-[-20] flex flex-col items-center gap-2"
        >
          <span className="text-lg font-mono">Role para ver mais</span>
          <ArrowDown className="w-5 h-5" aria-hidden="true" />
        </motion.div>
      </motion.div>
      </motion.section>

      
    </>
  );
};

export { Main };
